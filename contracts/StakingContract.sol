...
contract StakingPlatform {
  ...
  mapping(uint256 => Pool) public PoolsMapping;
   ...
   */
  function tokensPerBlock(uint256 _totalYieldTokens, uint256 _totalUsers, uint256 _totalTimeInSeconds) public view returns (uint256) {
      uint256 time = _totalTimeInSeconds / ChainBlockTime;
      uint256 yieldPerBlock = (_totalYieldTokens / _totalUsers) / time;
   ...
   */
  function createPool(IERC20 _stakedToken, 
                      IERC20 _rewardToken, 
    ...
    require(rewardToken.balanceOf(address(msg.sender)) >= _totalStakedAmount, 'Not enough reward tokens!');
